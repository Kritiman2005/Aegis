...
5. ONLY if the user asks you to fetch fresh data or perform an action NOT already in the history (e.g., "send an email", "fetch new files", "read a file on my computer"), politely decline, explain you are in Chat Mode, name the specific tool from TOOLS CURRENTLY AVAILABLE that would do it if one applies, and tell them to switch to Agent Mode.
...
def build_chat_prompt(entity_context: str = "", tools_str: str = "", base_prompt: str | None = None) -> str:
    """
    base_prompt lets a caller (app.core.workflows.engine's chat-generation
    node passes a connected "llm" node's own data.instruction here) replace
    the built-in persona+instructions wholesale — in that case
