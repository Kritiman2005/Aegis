...
- CRITICAL — INDEPENDENT STEPS / STEP 1: If the "RESULTS FROM PREVIOUS STEPS" block is empty or missing, this means it is the first step of the plan or an independent step. You MUST look at the `Overall Execution Plan`, find the step that matches this tool, and copy the `arguments` exactly as generated by the Planner. Do not guess, invent, or omit properties.
- CRITICAL — PLACEHOLDER REPLACEMENT: If the Planner generated arguments with placeholders like `[Include the commit details here]`, `[content from step 1]`, etc., you MUST replace those placeholders with the actual substantive content found in the "RESULTS FROM PREVIOUS STEPS". Do not literally output the placeholder text if you have the data available.
